import type { Settings, SummaryResult, TermsScores } from './types';
...
export function buildScoresPrompt(document: string): string {
...
export function parseScoresResponse(response: string): TermsScores | null {
  try {
    const clean = response.replace(/```json\s*/i, '').replace(/```\s*$/i, '').trim();
    const parsed = JSON.parse(clean) as Record<string, unknown>;

    function toCategory(val: unknown): { score: number; summary: string } {
      if (val && typeof val === 'object' && 'score' in val && 'summary' in val) {
        const obj = val as { score: unknown; summary: unknown };
        return {
