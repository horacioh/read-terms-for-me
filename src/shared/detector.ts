...
function scoreLink(element: HTMLAnchorElement, baseUrl: string): DetectedLink | null {
  ...
  if (termsRegex.test(text)) score += 10;
  if (hrefTermsRegex.test(lowerHref)) score += 8;
  if (lowerText.includes('terms of service')) score += 5;
  if (lowerText.includes('terms of use')) score += 4;
  if (lowerText.includes('user agreement')) score += 3;
  if (lowerHref.includes('/terms')) score += 3;
...
export function detectTermsLinks(document: Document, baseUrl: string): DetectedLink[] {
  const links = Array.from(document.querySelectorAll('a'));
  const scored: Map<string, DetectedLink> = new Map();
